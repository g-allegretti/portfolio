  ...
  return (
    <section id="experience" className="py-24 px-6 max-w-6xl mx-auto scroll-mt-16">
      <h2 className="text-3xl font-bold text-white mb-12">
        <span className="text-[#f59e0b] font-mono text-lg mr-3">02.</span>Experience
      </h2>

      <div className="md:flex gap-8">
        {/* Tab list */}
        <div className="md:w-52 flex md:flex-col mb-6 md:mb-0 border-b md:border-b-0 md:border-l border-[#2a2a2a] shrink-0">
          {experience.map((e, i) => (
            <button
              key={i}
              onClick={() => setActive(i)}
              className={`px-4 py-3 text-left text-sm transition-all whitespace-nowrap md:whitespace-normal border-b-2 md:border-b-0 md:border-l-2 -mb-px md:mb-0 md:-ml-px ${
                active === i
                  ...
                  : 'border-transparent text-slate-500 hover:text-slate-300 hover:bg-[#161616]'
              }`}
            >
              {e.company.split(' ')[0]}
            </button>
        ...
        <div className="flex-1 min-w-0 overflow-hidden">
          <div className="mb-1 flex flex-wrap items-baseline gap-2">
            ...
            <span className="text-[#f59e0b]">@ {item.company}</span>
          </div>
